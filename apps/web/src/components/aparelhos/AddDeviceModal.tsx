...
import { criarAparelhoSchema, type Categoria, type CriarAparelhoInput } from "@phonetrack/shared";
import { Modal } from "@/components/ui/Modal";
import { Button } from "@/components/ui/Button";
import { useCriarAparelho } from "@/hooks/use-aparelhos";
...
export function AddDeviceModal({
  ...
  const criarAparelho = useCriarAparelho(lojaId);
  ...
  async function onSubmit(data: CriarAparelhoInput) {
    setErro(null);
    try {
      await criarAparelho.mutateAsync(data);
      fechar();
    } catch {
    ...
    <Modal open={open} onClose={fechar}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-4 flex flex-col gap-3">
        ...
        <div className="mt-2 flex gap-2">
          ...
          <Button type="submit" className="flex-1" disabled={criarAparelho.isPending}>
            {criarAparelho.isPending ? "Adicionando..." : "Adicionar"}
          </Button>
        </div>
      </form>
    </Modal>
